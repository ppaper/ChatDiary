...
Ti.App.iOS.addEventListener('notification', function(data) {
	...
	//TODO:That currently make the second msg and so on won't be send
	if (isNewDay(lastQ1row, data.userInfo['date'])) {
		console.log("It's new day thou!!");
		db.execute('INSERT INTO chats (supervisor, date, msg, msg_type) VALUES (?,?,?,?)', data.userInfo['supervisor'], data.userInfo['date'].toISOString(), data.userInfo['msg'], data.userInfo['msg_type']);
		...
		Ti.App.fireEvent("msgGot", {
			id : msgId.fieldByName('id'),
			supervisor : data.userInfo['supervisor'],
			date : data.userInfo['date'].toISOString(),
			msg : data.userInfo['msg'],
			msg_type : data.userInfo['msg_type']
		});
	} else {
		console.log("......OP");
	}
...
Ti.App.addEventListener('isWriting', function() {
	//is writing msg
	Ti.API.info("User is writing something");
});

Ti.App.addEventListener('isNotWriting', function() {
	//cancel writing msg
	Ti.API.info("User is NOT writing something");
});

Ti.App.addEventListener("msgSent", function(msgData) {
	//Receive msg
	//TODO:Do ssomething when receive response
	var db = Ti.Database.open('diaryQA');
	Ti.API.info("MSG SENT!!");
	var currentQ_number = parseInt(msgData.msg_type.slice(1));
	var shall_Q_number = currentQ_number + 1;

	if (shall_Q_number <= 8) {
		var shall_Q_id = "q" + shall_Q_number;
		var day_time = new Date(msgData.date).getDay();
		console.log("day is " + day_time);
		var msg_content = db.execute("SELECT " + shall_Q_id + " FROM configs WHERE id=" + (parseInt(day_time) + 1));
		console.log("msg content is " + msg_content.field(0));
		if (msg_content.field(0) != null) {
			//there is further question
			console.log(msg_content.field(0));
			//send after 2 seconds
			var revoke_time = new Date(new Date().getTime()+120000);
			var timeToWrite = Ti.App.iOS.scheduleLocalNotification({
				date : revoke_time,
				alertBody : msg_content.field(0),
				userInfo : {
					supervisor : msgData.supervisor,
					msg : msg_content.field(0),
					date : revokeTime,
					msg_type : shall_Q_id
				}
			});
